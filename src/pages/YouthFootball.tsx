...
import { UserPlus, CalendarPlus, Plus, Users, Edit, Trash } from "lucide-react";
...
export default function YouthFootball() {
  ...
  const [activeView, setActiveView] = useState<"players" | "matches" | "materials">("players");
  ...
  const handleAddCategory = async (name: string) => {
    ...
    if (categoryToEdit) {
      ...
      const { error } = await supabase
        .from("categories")
        .insert([{ name, season_id: selectedSeason.id, organization_id: organizationId }]);
  ...
  const handleAddPlayer = async (playerData: any) => {
    ...
    try {
      const { error } = await supabase
        .from("players")
        .insert([{ ...playerData, category_id: selectedCategory, organization_id: organizationId }]);
  ...
  const handleAddCoach = async (coachData: Coach) => {
    try {
      ...
      const formattedCoachData = {
        ...
        category_id: coachData.category_id,
        organization_id: organizationId
      };
        ...
        try {
          ...
          if (authError) {
            ...
            const { error: updateError } = await supabase
              ...
              .eq('category_id', coachData.category_id);
  ...
  // Resetear al inicio de Fútbol Joven cuando se reciba el parámetro ?home=1
  useEffect(() => {
    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 relative">
        ...
        <div className="pt-32 p-8">
          <h1 className="text-2xl font-bold mb-4 mt-8">Fútbol Joven</h1>
          ...
          <div className="mt-8">
            ...
            <div className="flex gap-4 mb-4">
              ...
              >
                ...
                <div className="relative">
                  <span className={`text-sm font-rajdhani font-semibold uppercase tracking-wider ${activeView === 'materials' ? 'text-primary-foreground' : ''}`}>
