...
interface SpeedExcelUploaderProps {
  seasonId?: string;
  categoryId?: string;
...
export const SpeedExcelUploader = ({
  seasonId,
  categoryId,
  seniorSeasonId,
  seniorCategoryId,
}: SpeedExcelUploaderProps) => {
  ...
  const handleDeleteColumn = (headerToDelete: string) => {
    // Only allow users with edit permissions to delete columns
  ...
  const handleSaveDataset = async () => {
    ...
    const { error } = await supabase.from("speed_datasets").insert({
      name: datasetName,
      data: excelData,
    ...
    setDatasetName("");
    queryClient.invalidateQueries({ queryKey: ["speed-datasets"] });
  };
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Tooltip
                  ...
                  labelFormatter={(index: number, data: any) => data[index].name}
                />
