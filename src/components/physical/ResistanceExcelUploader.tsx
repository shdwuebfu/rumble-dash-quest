...
interface ResistanceExcelUploaderProps {
  seasonId?: string;
  categoryId?: string;
...
export const ResistanceExcelUploader = ({
  seasonId,
  categoryId,
  seniorSeasonId,
  seniorCategoryId,
}: ResistanceExcelUploaderProps) => {
  ...
  const handleSaveDataset = async () => {
    ...
    const { error } = await supabase.from("resistance_datasets").insert({
      name: datasetName,
      data: excelData,
    ...
    setDatasetName("");
    queryClient.invalidateQueries({ queryKey: ["resistance-datasets"] });
  };
