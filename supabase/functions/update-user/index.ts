...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (userData.staff_access !== 'editor') {
      return new Response(
        JSON.stringify({ error: 'No permission to edit users', access: userData.staff_access }),
        { status: 403, headers: corsHeaders }
      );
    }

    // Get request data
    let requestData: UpdateUserPayload;
    try {
    ...
    const { userId, userData: updateData } = requestData;
    ...
    // REMOVED: Don't allow users to edit themselves
    // This check has been removed to allow users to edit their own account

    // Extract password if provided
    const { password, ...permissionsData } = updateData;

    // 1. Update user metadata with permissions
    if (Object.keys(permissionsData).length > 0) {
      const { error: metadataError } = await adminClient.auth.admin.updateUserById(
        userId,
        {
          user_metadata: permissionsData
        }
      );
      ...
      if (metadataError) {
        ...
        return new Response(
          JSON.stringify({ error: `Error updating user metadata: ${metadataError.message}` }),
          { status: 500, headers: corsHeaders }
        );
      }

      // 2. Update the users table to match
      ...
      if (updateError) {
        ...
        return new Response(
          JSON.stringify({ error: `Error updating users table: ${updateError.message}` }),
          { status: 500, headers: corsHeaders }
        );
      }
    }
    ...
    if (password) {
      ...
      if (passwordError) {
        ...
        return new Response(
          JSON.stringify({ error: `Error updating password: ${passwordError.message}` }),
          { status: 500, headers: corsHeaders }
        );
      }
    }
