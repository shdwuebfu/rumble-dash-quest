-- Migration: fix_player_signup_2025_09_20_v2
-- Restaura alta de jugadores como el 16/09/2025 (idempotente + org correcta)

-- 1) Función y trigger: crear/actualizar jugador al crear auth.users
...
BEGIN
  ...
  INSERT INTO public.players (
    name, email, category_id, senior_category_id, user_id, is_auth_enabled,
    position, age, height, weight, jersey_number, image_url, organization_id
  ) VALUES (
    COALESCE(NEW.raw_user_meta_data->>'name', COALESCE(NEW.email, 'Jugador')),
    NEW.email, v_category_id, v_senior_category_id, NEW.id, true,
    NEW.raw_user_meta_data->>'position', v_age,
    NEW.raw_user_meta_data->>'height', NEW.raw_user_meta_data->>'weight',
    NEW.raw_user_meta_data->>'jersey_number', NEW.raw_user_meta_data->>'image_url',
    v_org_id
  )
...
-- 2) Función y trigger: sincronizar public.users con org derivada o fallback O'Higgins
...
BEGIN
  ...
  INSERT INTO public.users (
    ...
    NEW.id, NEW.email, COALESCE(NEW.raw_user_meta_data->>'name', NEW.email),
...
-- 3) Eliminar función existente con tipo de retorno json
DROP FUNCTION IF EXISTS public.handle_player_email_reuse(text,text,integer,text,text,text,text,text,uuid,uuid,text);

-- 4) Crear nueva función con tipo de retorno json (mantener compatibilidad)
...
BEGIN
  ...
  IF v_user_id IS NULL THEN
    RETURN json_build_object('success', true, 'create_new_user', true);
  END IF;
  ...
  IF v_existing_player IS NOT NULL THEN
    RETURN json_build_object('success', false, 'error', 'email_in_use',
  ...
  INSERT INTO public.players (
    ...
    COALESCE(p_name, p_email), p_email, p_category_id, p_senior_category_id,
    v_user_id, true, p_position, p_age, p_height, p_weight, p_jersey_number, p_image_url, v_org_id
  )
  ON CONFLICT (user_id) DO NOTHING;

  RETURN json_build_object('success', true, 'reused_account', true);
END;
$$;